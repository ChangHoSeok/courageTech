...
jQuery.url = function() {
	...
	var parseUri = function() {
		...
		uri[options.hash.name] = {};
		uri[options.key[13]].replace(options.hash.parser, function($0, $1, $2) {
			if ($1) {
				uri[options.hash.name][$1] = $2;
			}
		});
	...
	var hashParam = function(item) {
		if (!parsed.length) {
			setUp(); // if the URI has not been parsed yet then do this first...
		}
		return (parsed.hashKey[item] === null) ? null : parsed.hashKey[item];
	};
	...
	return {
		...
		hashParam : hashParam // provides public access to private 'param' function - see above
